...
function HueWheel(elementID, shapes, target_angle, locationAngles, probe, probeByLocation, random_rotation, callbackFunction, jitters) {
    ...
    function clear() {
        ...
        if (showColor) {
            if (probeByLocation) {
                stimuliDisplay.drawShapesProbeByLocation(canvas, shapes, [r, g, b], locationAngles, Array.from({ length: shapes.length }, (v, i) => i), false, probe, jitters);
            } else {
                stimuliDisplay.drawShapesProbeByShape(canvas, shape, [r, g, b], false);
            }
        } else {
            if (probeByLocation) {
                stimuliDisplay.drawShapesProbeByLocation(canvas, shapes, [255, 255, 255], locationAngles, Array.from({ length: shapes.length }, (v, i) => i), false, probe, jitters);
            } else {
